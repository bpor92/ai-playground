<template>
  <UiForm>
    ...
    <UiDivider>Candidate</UiDivider>

    <div class="w-full p-2 md:max-w-sm md:mx-auto">
      <UiElText v-model="form.candidate" label="Surname and name" />
      <UiElText v-model="form.position" label="Position" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
      <div v-for="item in form.questions" :key="item.id" class="mb-5 self-end">
        <UiElTextarea v-model="item.value" :label="item.text" :loading="item.loader" />
        <div v-if="ratedQuestions">rate: {{ item.rate }}</div>
      </div>
    </div>
...
import { UiDivider } from '~/.nuxt/components'
